...
class Ned2_control(object):
    def __init__(self):
        ...
        group_name = "h2017" #moveit의 move_group name >> moveit assitant로 패키지 생성 시 정의
    ...
    def step(self, angle):
        ...
        elif(df > distance >= df*0.7): self.weight = 15
        elif(df*0.7 > distance >= df*0.5): self.weight = 10
        elif(df*0.5 > distance >= df*0.1): self.weight = 5
        elif(df*0.1 > distance >= 0): self.weight = 3
        ...
        linear_velocity = self.get_end_effector_linear_velocity(current_pose, previous_pose, 23.3*(end-start))
